package org.mayangwy.admin.core.ext.druid;
...
import org.mayangwy.admin.core.utils.SqlUtils;
...
public class LogRecordFilter extends FilterEventAdapter {
    ...
    public static final ThreadLocal<String> threadSqlLevel = new ThreadLocal<>();
     ...
     *
     */
    @Value("${spring.datasource.druid.filter.logRecord.sqlLevel:NONE}")
    ...
    private LogType getPrintLevel(){
        /*String threadName = Thread.currentThread().getName();
        if(threadName != null && threadName.contains("AdminSchedulerNoSqlPrint")){
            return LogType.NONE;
        }*/
        String s = ObjectUtil.defaultIfNull(threadSqlLevel.get(), sqlLevel);
        if(LogType.INFO.name().equals(s)){
            return LogType.INFO;
        } else if(LogType.DEBUG.name().equals(s)){
